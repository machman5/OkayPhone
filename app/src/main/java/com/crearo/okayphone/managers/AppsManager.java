...
//                todo support delete
    ...
    private List<LaunchInfo> createAppMap(PackageManager mgr) {
        ...
        for (ResolveInfo ri : main)
            infos.add(new LaunchInfo(ri.activityInfo.packageName, ri.activityInfo.name, ri.loadLabel(mgr).toString()));
    ...
    public static class LaunchInfo implements Compare.Stringable, Parcelable {
        ...
        public boolean is(String app) {
            ...
            if (split2.length == 1) {
                ...
                if (componentName.getPackageName().equals(split2[0]) && componentName.getClassName().equals(split2[1]))
                    return true;
            }
